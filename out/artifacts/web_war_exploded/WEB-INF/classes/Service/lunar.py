...
class Lunar(object):
    ...
    def ln_date_str(self):# 返回农历日期字符串，形如：农历正月初九
        ...
        year = list(str(year))
        year = [self.ly[int(i)] for i in year]
        return '农历 {}年 {}月 {}'.format(''.join(year), self.lm[month-1], self.ld[(day-1)*2:day*2])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', '--today', action='store_true', help='whether is today')
    parser.add_argument('-y', '--year', type=int, default=1901, help='input the year')
    parser.add_argument('-m', '--month', type=int, default=1, help='input the month')
    parser.add_argument('-d', '--day', type=int, default=1, help='input the day')

    ARGS = parser.parse_args()
    if ARGS.today:
        ct = datetime.datetime.now()
        # ct = datetime.datetime(now.year, now.month, now.day)
    else:
        ct = datetime.datetime(ARGS.year,ARGS.month,ARGS.day)
    Service.test(ct)
